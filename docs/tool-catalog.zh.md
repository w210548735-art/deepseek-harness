...
| `@deepseek-ai/dsh-tool-session-collaboration` | `session_delegate` | `ctx.tools`、`ctx.systemPrompt`、`ctx.sessionCollaboration`、`a calling Agent for delivery authority` | `tool/call`、`tool/result`、`session-collaboration relay events` | - | `session_delegate` 按明确的现有会话 ID 寻址，通过 next-step steering 发送模型上下文，并可将目标回复注入调用方。跨工作区投递仍然只接受明确 ID，目录采集时才启用。 |
...
向明确指定的现有会话发送模型上下文消息，通过 next-step steering 投递，并可选返回目标回复。
...
```json
{
  ...
  "properties": {
    "session_id": {
      "type": "string",
      "description": "Target session id supplied by the user or synchronized by a prior collaboration relay."
    },
    "prompt": {
      "type": "string",
      "description": "Message or task to deliver as model context to the target session."
    },
    "wait": {
      "type": "boolean",
      "description": "Wait for the target turn reply; defaults to true. Use false for asynchronous messages."
    }
  },
  "required": [
    "session_id",
...
来源：[`packages/session-query/tool-session-collaboration/src/index.ts`](../packages/session-query/tool-session-collaboration/src/index.ts)

`session_delegate` 按明确的现有会话 ID 寻址，通过 next-step steering 发送模型上下文，并可选将目标回复注入调用方。跨工作区投递仍然只接受明确 ID，目录采集时才启用。
